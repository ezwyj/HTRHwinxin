define(['common', 'util', 'plugins'], function ($) {
    ...
    //选择人员
    $(document).on('click', '.selectPeople', function () {
        resultContainer = $(this).next();
        ...
        $.content({
            ...
            content: {
                width: 880,
                height: 480,
                src: OP_CONFIG.selectorUrl + 'Selector/MultiPeople?proxyPage=' + encodeURIComponent(OP_CONFIG.proxyUrl + 'Common/ProxyPage?callback=selectPeople')
            }
        });
    });
    ...
    window.selectPeople = function (data) {
        ...
        // IE8抽风，不能用forEach，原因是没有识别到data是数组
        for (var i in data) {
            html += '<span class="person-item" data-badge="' + data[i].Badge + '">' + data[i].Name + '</span>';
        }
    ...
    $('#add-set').on('click', function (e) {
        $.content({
            ...
            content: {
                width: 400,
                html: '<div class="item">' +
                            '<div class="item-name"><span class="required">*</span>集名称：</div>' +
    ...
    $('#add-value').on('click', function () {
        ...
        var html = '<div class="item">' +
                        '<div class="item-name"><span class="required">*</span>值：</div>' +
                        '<div class="item-value"><input type="text" name="Value" class="form-control" /></div>' +
                    '</div>' +
                    '<div class="item">' +
                        '<div class="item-name"><span class="required">*</span>文本：</div>' +
                        '<div class="item-value"><input type="text" name="Text" class="form-control" /></div>' +
                    '</div>' +
                    '<div class="item">' +
                        '<div class="item-name">排序：</div>' +
                        '<div class="item-value"><input type="text" name="SortId" class="form-control" placeholder="请输入数字" /></div>' +
                    '</div>' +
                    '<div class="item">' +
                        '<div class="item-name">是否启用：</div>' +
                        '<div class="item-text">' +
                            '<label><input type="radio" name="IsEnable" value="1" checked /> 是</label>&nbsp;&nbsp;&nbsp;&nbsp;' +
                            '<label><input type="radio" name="IsEnable" value="0" /> 否</label>' +
                        '</div>' +
                    '</div>' +
                    '<div class="item">' +
    ...
    $(document).on('click', '#valueList .value-setting', function () {
        ...
        $.get(rootUrl + 'System/GetValueSetById?id=' + id, function (res) {
            if (res.state) {
                ...
                var html = '<div class="item">' +
                                '<div class="item-name"><span class="required">*</span>值：</div>' +
                                '<div class="item-value"><input type="text" name="Value" class="form-control" value="' + data.Value + '" /></div>' +
                            '</div>' +
                            '<div class="item">' +
                                '<div class="item-name"><span class="required">*</span>文本：</div>' +
                                '<div class="item-value"><input type="text" name="Text" class="form-control" value="' + data.Text + '" /></div>' +
                            '</div>' +
                            '<div class="item">' +
                                '<div class="item-name">排序：</div>' +
                                '<div class="item-value"><input type="text" name="SortId" class="form-control" value="' + data.SortId + '" placeholder="请输入数字" /></div>' +
                            '</div>' +
                            '<div class="item">' +
                                '<div class="item-name">是否启用：</div>' +
                                '<div class="item-text">' +
                                    '<label><input type="radio" name="IsEnable" value="1"' + (data.IsEnable ? ' checked' : '') + ' /> 是</label>&nbsp;&nbsp;&nbsp;&nbsp;' +
                                    '<label><input type="radio" name="IsEnable" value="0"' + (data.IsEnable ? '' : ' checked') + ' /> 否</label>' +
                                '</div>' +
                            '</div>' +
                            '<div class="item">' +
                                '<div class="item-name">是否默认：</div>' +
                                '<div class="item-text">' +
                                    '<label><input type="radio" name="IsDefault" value="1"' + (data.IsDefault ? ' checked' : '') + ' /> 是</label>&nbsp;&nbsp;&nbsp;&nbsp;' +
                                    '<label><input type="radio" name="IsDefault" value="0"' + (data.IsDefault ? '' : ' checked') + ' /> 否</label>' +
                                '</div>' +
                            '</div>';
    ...
    $('#addUserRight').on('click', function () {
        $.content({
            ...
            content: {
                width: 400,
                html: '<div class="item">' +
                            '<div class="item-name">员工：</div>' +
                            '<div class="item-value">' +
                                '<button class="btn btn-info btn-xs selectPeople">选择人员</button>' +
                                '<div class="persons"></div>' +
                            '</div>' +
                        '</div>' +
                        '<div class="item">' +
                            '<div class="item-name"><span class="required">*</span>权限：</div>' +
                            '<div class="item-text right-list">' +
                                '<label><input type="checkbox" class="form-contorl" value="提交缺陷" > 提交缺陷</label>' +
                                '<label><input type="checkbox" class="form-contorl" value="权限设置" > 权限设置</label>' +
                                '<label><input type="checkbox" class="form-contorl" value="值集维护" > 值集维护</label>' +
                                '<label><input type="checkbox" class="form-contorl" value="审批列表" > 审批列表</label>' +
                                '<label><input type="checkbox" class="form-contorl" value="日志查询" > 日志查询</label>' +
                                '<label><input type="checkbox" class="form-contorl" value="报表导出" > 报表导出</label>' +
                            '</div>' +
                        '</div>'
            },
            footer: [{
                ...
                callback: function () {
                    ...
                    $('.persons .person-item', this).each(function () {
                        users.push($(this).data('badge'));
                    });
            ...
            if (res.state) {
                ...
                $.content({
                    ...
                    content: {
                        ...
                        html: '<div class="item">' +
                                    '<div class="item-name">员工：</div>' +
                                    '<div class="item-text">' + data.RightUserExp + '</div>' +
                                '</div>' +
                                '<div class="item">' +
                                    '<div class="item-name"><span class="required">*</span>权限：</div>' +
                                    '<div class="item-text right-list">' +
